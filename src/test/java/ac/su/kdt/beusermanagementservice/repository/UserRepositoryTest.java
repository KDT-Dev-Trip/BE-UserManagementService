...
class UserRepositoryTest {
    ...
    @Test
    @DisplayName("사용자 저장 및 authUserId로 조회 테스트")
    void saveAndFindByAuth0Id() {
        ...
        // then
        // userRepository의 findByAuthUserId 메서드를 호출하여 방금 저장한 사용자를 다시 조회
        User foundUser = userRepository.findByAuthUserId("auth0|test-user-123").orElseThrow();

        // 조회된 사용자의 authUserId가 처음에 설정한 authUserId와 같은지 확인
        assertThat(foundUser.getAuthUserId()).isEqualTo(newUser.getAuthUserId());
